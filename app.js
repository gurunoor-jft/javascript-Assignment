...
function display() {
  ...
  jsonData.forEach((jsonD) => {
    ...
    temp +=
      '<td><button style="color:red" type="button" class="fas fa-trash" data-toggle="modal" data-target="#DeleteButton" onclick=store(' +
      jsonD["id"] +
      ")></button></td>";
    temp +=
      '<td><button style="color:DeepSkyBlue" type="button" class="fas fa-edit" data-toggle="modal" data-target="#EditButton" onclick=editFun(' +
      jsonD["id"] +
      ")></button></td>";
    temp += "</tr>";
  });
...
function addFunction() {
  ...
  if (Name === null || Name === "") {
    ...
    document.getElementById("Email").style.border =
      "rgb(230, 28, 28) 1px solid";
    return false;
  } else if (!Email.match(mailformat)) {
    document.getElementById("addNameSpan").innerHTML = "";
    document.getElementById("addEmailSpan").innerHTML =
      "Please Fill Email in xxx@gxxx.com format";
    document.getElementById("Name").style.border = "black 1px solid";
    document.getElementById("Email").style.border =
      "rgb(230, 28, 28) 1px solid";
    return false;
